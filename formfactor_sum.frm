...
#call Color
if (count(T2, 1) == 3);
    id T2(cOli1?) * T2(cOli2?) * T2(cOli3?) = - T1(cOli3) * T1(cOli2) * T1(cOli1); 
else if (count(T2, 1) == 2) ; 
    id T2(cOli1?) * T2(cOli2?) = T1(cOli2) * T1(cOli1);
else if (count(T2, 1) == 1); 
    id T2(cOli1?) = -T1(cOli1); 
endif; 
id sumi =1; 
id sumj = 1; 
id sumij = 1;
...
.sort
.store 
save results/formfac.sav diags1l;
